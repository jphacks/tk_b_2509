...
export default function Header() {
  ...
  return (
    <header className="bg-white border-b border-slate-200 shadow-sm sticky top-0 z-50">
      <div className="max-w-full px-4 py-3 md:px-6 md:py-5">
        {/* スマホ: ロゴ中央レイアウト */}
        ...
        <div className="hidden md:grid md:grid-cols-3 items-center gap-4">
          ...
          <div className="flex justify-center">
            {!isRoot && (
              <h1 className="md:text-xl lg:text-2xl font-bold text-slate-900 truncate">
                {getPageTitle()}
              </h1>
            )}
          </div>
          ...
          <div className="flex justify-end items-center gap-3">
            {isRoot && (
              <>
                <Link
                  href={ROUTES.login}
                  className="md:text-sm lg:text-base font-medium text-slate-700 hover:text-slate-900"
                >
                  ログイン
                </Link>
                <Link
                  href={ROUTES.signup}
                  className="md:px-3 md:py-1.5 lg:px-4 lg:py-2 rounded-md bg-blue-600 text-white md:text-sm lg:text-base font-medium hover:bg-blue-700"
                >
                  会員登録
                </Link>
              </>
