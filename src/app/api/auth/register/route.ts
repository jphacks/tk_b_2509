...
import { hashPassword, generateToken, createAuthSuccessResponse } from "@/lib/auth";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (password.length < 6) {
      return NextResponse.json(
        { error: "パスワードは6文字以上で入力してください", code: "PASSWORD_TOO_SHORT" },
        { status: 400 }
      );
    }
    ...
    if (existingUser) {
      return NextResponse.json(
        { error: "このユーザー名は既に使用されています", code: "USERNAME_EXISTS" },
        { status: 409 }
      );
    }
